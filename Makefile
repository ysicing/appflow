...
IMG ?= ysicing/appflow:${IMG_VERSION}
...
.PHONY: build
build: default generate ## Build manager binary.
...
.PHONY: docker
docker: test ## Build and push docker image with the manager.
	docker buildx build --pull --push  --platform linux/amd64  -t ${IMG} .
